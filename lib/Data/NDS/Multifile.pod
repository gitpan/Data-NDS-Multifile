...
If the data files contains lists of elements, $label must be passed
in, but $ele is NOT passed in.

If the data files contain hashes of elements, $ele is passed in, but
$label is NOT passed in.
...
This inserts the element at the end of the list contained in the
given file label.
...
If $label is given, it is inserted into the last data file. Otherwise,
it is inserted into the file specifed by $label.
